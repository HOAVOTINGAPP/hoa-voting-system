...
def vote_topic(topic_id):
    ...
    if not topic or not topic["is_open"]:
        conn.close()
        template = BASE_HEAD + "<h1>Voting not available for this topic.</h1>" + BASE_TAIL
        return render_template_string(template)
    ...
    if request.method == "POST":
        ...
        conn.commit()
        conn.close()
        template = BASE_HEAD + "<h1>Thank you, your vote has been recorded.</h1>" + BASE_TAIL
        return render_template_string(template)
...
def admin_export():
    ...
    # Build CSV as text then send as bytes
