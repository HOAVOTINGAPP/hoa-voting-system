...
def admin_registrations():
    ...
    # owner proxies (used only to calculate blocking / total weight here)
    ...
    # compute per-ERF total vote weight (matching export logic)
    regs = []
...
<div class="card">
  ...
  <p class="subtle">
    ...
    (e.g. "this owner holds 3 additional proxy votes").<br>
    Use the <strong>Owner Proxy Allocator</strong> tab for linking specific ERF numbers as proxies.
  </p>
  <form method="post">
  ...
  <table>
      ...
      {% endfor %}
      <tr>
          <td colspan="3"><strong>Total vote weight (all ERFs)</strong></td>
...
def admin_owner_proxies():
    """
    New tab: Owner Proxy Allocator.
    Here the admin can pre-allocate proxies by linking ERFs before the meeting.
    """
    if not admin_logged_in():
        return redirect(url_for("admin_login"))
...
""" + BASE_TAIL
    return render_template_string(template, owner_proxies=owner_proxies_rows)
