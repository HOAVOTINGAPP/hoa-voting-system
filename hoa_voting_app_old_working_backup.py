...
<div class="card">
  ...
  <p class="subtle">Place the cursor in the ERF box and scan the owner's ID/ERF. The scanner will type the ERF and press Enter to submit.</p>
  <p class="subtle">
...
# DUPLICATE_ROUTE_REMOVED: original decorator commented out to avoid Flask collision
...
# DUPLICATE_ROUTE_COMMENTED_OUT @app.route("/admin/owner_proxies/add_ajax", methods=["POST"])
def admin_owner_proxies_add_ajax():
    if not admin_logged_in():
        return jsonify({"success": False, "message": "unauthorized"}), 403
    primary_erf = request.form.get("primary_erf", "").strip().upper()
...
# DUPLICATE_ROUTE_COMMENTED_OUT @app.route("/admin/owner_proxies/delete_ajax", methods=["POST"])
def admin_owner_proxies_delete_ajax():
    if not admin_logged_in():
        return jsonify({"success": False, "message": "unauthorized"}), 403
    proxy_id = request.form.get("owner_proxy_id", "").strip()
