...
<h1>Admin Login</h1>
<form method="post">
    <p><label>Password: <input type="password" name="password" autofocus required></label></p>
...
<form method="post">
    ...
    <p><label>One-Time PIN: <input type="password" name="otp" required></label></p>
    <button type="submit">Login</button>
</form>
""" + BASE_TAIL
    return render_template_string(template)
...
def vote_topic_selector():
    ...
    template = BASE_HEAD_PUBLIC + """
<h1>Owner Voting</h1>
<p>Logged in as ERF {{ voter_erf }}. <a href="{{ url_for('vote_logout') }}">Logout</a></p>
{% if not topics %}
